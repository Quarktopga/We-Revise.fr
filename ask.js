...
function getResponse(rawQuery) {
    ...
    if (includesAny(q, ["ajouter", "importer", "mettre"]) &&
        includesAny(q, ["fiche", "pdf", "image", "jpeg", "png"])) {
        return pick([
    ...
    // Matières
    if (includesAny(q, ["matiere", "matieres", "liste", "cours", "discipline"])) {
        return pick([
    ...
    // Chapitres
    if (includesAny(q, ["chapitre", "chapitres"])) {
        return pick([
    ...
    // Fiches
    if (includesAny(q, ["fiche", "fiches", "pdf", "document", "image"])) {
        return pick([
    ...
    // Feedback
    if (includesAny(q, ["feedback", "avis", "message", "commentaire"])) {
        return pick([
    ...
    // Entraînement
    if (includesAny(q, ["entrainement", "entrainements", "quiz", "exercice"])) {
        return pick([
    ...
    // ASK lui-même
    if (includesAny(q, ["ask", "assistant", "aide", "bot", "ia"])) {
        return pick([
